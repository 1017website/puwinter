    ...
    <section class="hero">
        <div class="hero-shell">
            ...
            <div class="hero-visual">
                <div class="dashboard">
                    <div class="dash-top"><strong>Kelas Online Berlangsung</strong><span class="dash-pill">LIVE</span></div>
    ...
    <div id="harga" class="pricing-section">
        ...
        <div class="pricing-grid">@forelse($plans as $plan)
            ...
            @endphp<div class="pricing-card {{ $plan->is_popular ? 'popular' : '' }} reveal">@if($plan->is_popular)<div class="popular-tag">PALING POPULER</div>@endif<div class="pricing-name">{{ $plan->name }}</div>
                ...
                @if($plan->periodLabel())<div style="font-size:11px;color:#94A3B8;font-weight:600;margin-top:2px;"><i class="far fa-calendar-alt"></i> {{ $plan->periodLabel() }}</div>@endif @if($plan->original_price > $plan->price)<div class="pricing-strike">Rp {{ number_format($plan->original_price) }}{{ $discount }}</div>@endif @if($plan->bonus)<div style="font-size:12px;color:var(--accent);font-weight:800;margin:6px 0;">🎁 {{ $plan->bonus }}</div>@endif @if($plan->features)<ul class="pricing-features">@foreach($plan->features as $f)<li><i class="fas fa-check-circle"></i> {{ $f }}</li>@endforeach</ul>@endif<a href="{{ route('register') }}" class="btn-pricing {{ $plan->is_popular ? 'btn-pricing-filled' : 'btn-pricing-outline' }}">{{ $plan->is_popular ? 'Pilih Program Ini' : 'Daftar Sekarang' }}</a>
    ...
    <div id="testimoni" class="testi-section">
        <div class="testi-inner">
            ...
            <div class="testi-grid">@foreach([
                ['initial'=>'A','name'=>'Aditya Pratama','info'=>'English Speaking Program Student','color'=>'#5337ec','text'=>'"Puwinter benar-benar game changer. Kelas online-nya interaktif banget, bisa langsung tanya kalau ada yang gak ngerti. Speaking dan writing saya meningkat pesat dalam 2 bulan!"'],
